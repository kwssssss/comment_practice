		...
		<div class="comment my-3" data-no="${comment.no}" data-writer="${comment.writer}">
			<div class="comment-title my-2 d-flex justify-content-between">
				<div>
					...
					</strong> <span class="text-muted ml-3 comment-date">
...
function showUpdateComment(e) {
	...
	const commentEl = $(this).closest('.comment'); //closet- 부모에서 찾는거
	const no = commentEl.data("no");
	
	const contentEl = commentEl.find('.comment-content'); //find- 자식에서 찾는거
	const comment = { no, content: contentEl.html().trim() }; //trim - 앞 뒤 공백 제거
